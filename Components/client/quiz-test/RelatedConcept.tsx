...
export default function RelatedConcept({
  ...
  return (
    <div
      ...
      className="flex flex-col w-[30%] max-md:w-[90%] rounded-2xl bg-white dark:bg-[#1e1e1e] shadow-[0_0_6px_rgba(0,0,0,0.2)] dark:shadow-[0_0_8px_rgba(0,0,0,0.5)]    hover:-translate-y-2 hover:shadow-[0_10px_25px_rgba(0,0,0,0.15)] dark:hover:shadow-[0_10px_25px_rgba(0,0,0,0.45)]
                  transition-all duration-300 ease-out cursor-pointer"
    >
      {/* Header */}
      <div
        className={`flex flex-col items-start ${color.bg} dark:bg-[#303030] px-6 pt-4 rounded-t-2xl min-h-[90px]`}
      >
        <p className="bg-[#FFFFFF80] dark:bg-[#ffffff15] text-xs px-3 rounded-full py-1 capitalize text-gray-800 dark:text-gray-200">
          {post.subject}
        </p>

        <p className="text-md font-bold py-3 line-clamp-2 text-gray-900 dark:text-white">
          {post.title}
        </p>
      </div>

      {/* Footer */}
      <div className="flex flex-row gap-4 relative py-4 px-6 rounded-2xl bg-white dark:bg-[#1e1e1e]">
        ...
        {/* Arrow */}
        <div className="absolute -top-5 right-6 p-2 bg-white dark:bg-[#303030] shadow-[0_0_6px_rgba(0,0,0,0.2)] dark:shadow-[0_0_6px_rgba(0,0,0,0.6)] rounded-full">
      ...
      {session?.user?.role === "ADMIN" && (
        <div
          ...
          className="flex justify-around border-t border-[#e5e5e5] dark:border-[#3d3d3d] py-2 text-sm"
        >
