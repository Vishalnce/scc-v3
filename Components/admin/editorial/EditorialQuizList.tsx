...
const EditorialQuizList = ({ id, setQuesId }: QuestionWrapperProps) => {
  ...
  const fetchQuestions = () => {
    ...
    fetch(`/api/en/editorial-quiz/admin?postId=${id}`)
      .then((res) => {
    ...
    {questions.map((q) => (
      <li key={q.id} className="flex justify-between items-start border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition">
        ...
        <div className="flex flex-col ml-6 space-y-3">
          ...
          <EditorialDeleteButton quesId={q.id} onDeleted={() => fetchQuestions()} />
        </div>
      </li>
    ))}
  </ul>
...
export default EditorialQuizList;
