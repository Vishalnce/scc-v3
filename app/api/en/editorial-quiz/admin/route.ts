...
export async function POST(req: Request) {
  try {
    ...
    const quiz = await db.editorialQuiz.create({
...
export async function GET(req: Request) {
  try {
    ...
    if (quesId) {
      ...
      const question = await db.editorialQuiz.findUnique({
        where: { id: quesId },
      });
    ...
    if (postIdParam) {
      ...
      const questions = await db.editorialQuiz.findMany({
        where: { editorialId: postId },
...
export async function PATCH(req: Request) {
  try {
    ...
    const updatedQuestion = await db.editorialQuiz.update({
      where: { id: quesId },
      data: updateData,
    });
