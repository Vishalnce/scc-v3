...
async function fetchUpcomingExam(pageNumber: number): Promise<FetchResponse> {
  try {
    const res = await fetch(
...
// main components
export default async function UpcomingExamPage({
  params,
  searchParams,
}: {
  params: Promise<{ slug: string }>;
  ...
  const { posts, page } = await fetchUpcomingExam(pageNumber);
