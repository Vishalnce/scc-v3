...
import QuizWrapper from "@/Components/client/editorial-quiz/QuizWrapper";
...
async function fetchCurrentAffairs(): Promise<FetchResponse> {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/en/editorial/client/?limit=5`, {
      cache: "no-store",
    });
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      url: `${process.env.NEXT_PUBLIC_SITE_URL}/api/en/editorial-page/client/${slug}`,
    ...
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_SITE_URL}/api/en/editorial-page/client/${slug}`,
    },
    robots: {
      index: true,
      follow: true,
...
export default async function CurrentAffairsPage({
  ...
  async function fetchNextCurrentAffairs(page: number) {
    try {
      const res = await fetch(`/api/en/editorial/client/?page=${page}`, {
        cache: "no-store",
      });
    ...
    <>
      ...
      <div className="bg-white dark:bg-black pt-12 pb-12">
        <div className="w-[90%] relative  mx-auto flex flex-row gap-10 justify-between">
          {/* left box */}
          <div className="w-[30%]  flex flex-col gap-4  max-md:hidden sticky top-40 self-start h-fit ">
            {/* table of content */}
