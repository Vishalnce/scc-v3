...
import DeleteButton from "@/Components/client/editorial/DeleteButton";
import EditButton from "@/Components/client/editorial/EditButton";
import CurrentAffairsFilter from "@/Components/client/editorial/CurrentAffairsFilter";
...
async function fetchPosts(
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_SITE_URL}/api/en/editorial/client?${params.toString()}`,
    {
      cache: "no-store",
    },
  );
...
export default async function Page({searchParams,}: {searchParams: Promise<{page?: string;topic?: string;date?: string;limit?: string;}>;
  ...
  try {
    const data = await fetchPosts(page, limit, topic, date);
    posts = data.posts;
        ...
        <div className="flex flex-col justify-center items-center  max-md:min-h-[80px] md:min-h-[150px] mx-auto max-w-[1400px] max-sm:w-[90%] text-center">
          <h1 className="text-4xl font-bold max-sm:text-2xl">
            <p className="text-center">
              ...
              <span className="max-md:hidden"> Success </span>
            </p>
          </h1>
          <p className="mt-1 text-lg max-md:text-sm text-my-text-color text-center">
            Stay Ahead with Daily Updates{" "}
            <span className="max-md:hidden">
              {" "}
              Tailored for SSC CGL General Awareness!
            </span>
          </p>
        </div>
      </header>
      ...
      <div className="dark:bg-[#191919]">
        ...
        {session?.user?.role === "ADMIN" ? (
          <div className="w-[90%] dark:bg-[#191919] mx-auto m-6 max-md:hidden">
            <Link href="/admin/editorial-editor ">
            ...
            posts.map((post, index) => {
              ...
              return (
                ...
                >
                  <Link
                    href={{
                      pathname: `/editorial-page/${post.slug}`,
                      query: { page: page },
                    }}
